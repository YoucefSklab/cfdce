...
public class ResolverTimeConsuming {
	...
	public static void main(String[] args) throws IOException, FIPAException, InterruptedException {
		...
		while (!end) {
			 ...
			 System.out.println(" -> Indice Max: " + tempIndiceMax.toString() + " - Step:"
			 + step);
			 System.out.println(" -> Indice Min: " + tempIndiceMin.toString() + " - Step:"
			 + step);
			...
			for (int i = 0; i < agentList.size(); i++) {
				...
				for (int j = 0; j < agent.discussionList.size(); j++) {
					...
					//uConsumedTime += ThreadLocalRandom.current().nextInt(minVal, maxVal);
				}
		...
		System.out.println(" 	-> Initial needed steps: " + neededSteps + " and total needed time is "
				+ (neededSteps * totalAgentUtilityTime));
		...
		System.out.println("Total consumed time in seconds is: "+ (globalElapsedTime / 1));
		System.out.println("Total utility consumed time is: "+ (uConsumedTime / 1));
		System.out.println("Total needed time all alternatives is " + (neededSteps * totalAgentUtilityTime));
		...
		long allValue = 0;
		/*
		 * for (int i = 0; i < neededSteps; i++) { System.out.println(neededSteps +
		 * " ---> " + i + " reste: " + (neededSteps - i)); allValue += rand.nextInt(); }
		 */
