...
public class ResolverV3 {
	...
	public static long					startingTime		= System
			.currentTimeMillis();
...
/*
		PrintStream out = new PrintStream(new FileOutputStream("logs/" + System.currentTimeMillis()
						+ "_Resolver_Log" + agentNames + ".txt"));
		System.setOut(out);
...
*/		
		...
		for( int i = 0; i < allTasksList.size(); i++ ) {
			...
			task.combPossibilities = MethodesCollection
					.formCoalitionStr(task.agentList, true);
		...
		while (!end) {
			...
			for( int i = 0; i < allTasksList.size(); i++ ) {// //System.out.println("
															// - 1");
				Task task = (Task) allTasksList.get(i);
				// System.out.println();
				ArrayList liste = task
						.getPossibilitiesFromStr((int) indiceMin.get(i));
				// System.out.println(" --> The task: "+task.task+" with
				// selected list: "+liste.toString());
				System.out.println("  --> The task: " + task.task
						+ "  with selected list(" + (int) indiceMin.get(i)
						+ "): " + liste.toString()
				...
				if (liste.size() >= 2) { // ex�cution collective
					...
					MethColl.setCollectiveExecution(agentList, liste,
							task.task);
		...
		System.out.println(" --> Final Steps: " + limit);
