...
public class ResolverV7 {
	...
	public static int[]					agentTab			= {1,2,3,4};
	// public static Logger 0logger = LoggerFactory.getLogger(ResolverV3.class);
	...
	public static void main(String[] args)
		...
		System.out.println(" 	-> indicePlan: " + indicePlan);
		System.out.println(" 	-> The set V: " + totalTeT);
		...
		while (!end) {
			...
			while (cmp > 0) {
				...
				if ((int)tempIndiceMin.get(cmp) >= (int) tempIndiceMax.get(cmp)) {
					...
					/*
					System.out.println("				-> Donc il faut revoir la liste des alternatives de "+(cmp)+" � "+(allTasksList.size()-1)+".");
					System.out.println("				-> Les t�ches concernc�es sont: ");
					
					for(int h=cmp; h<allTasksList.size(); h++){
						...
						for(int y=0; y<cmp; y++){
							...
							if(task2.exclusiveTasks.contains(task.task)){
								System.out.println("						-> "+task2.task+ " Sa position est "+y+  " est la position d'alternative est "+(int) tempIndiceMin.get(y)+" Total de ses alternatives est: "+task2.combPossibilities.size());
						...
						System.out.println("					-> "+task.task +" : Exclure ces agents: "+updatedAgentList.toString());
						System.out.println("						-> : Ses anciennes possibiolit�s: "+task.combPossibilities.toString());
