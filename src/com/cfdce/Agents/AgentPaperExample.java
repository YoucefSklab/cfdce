 ...
 *  
 */
public class AgentPaperExample extends Agent {
	...
	//	public Hashtable discussionsProbability=new Hashtable(); 
		...
		// {w, eCost, rf, sf, conv}
		public int val[] = {2,20,5,7,10}; 
		//public int val[] = {2,20,5,7,15}; j'ai une solution
		...
		private int maxRound = 8;//12;  // nombre max de round � ex�cuter 
		private float costLimitPercentage = 90; //77
		...
		private int mainWaitingTime = 2000; //9000
		...
		public static long allProcessNeededTime = 35000;//40000; // 50000
		...
		private int GlobalStep = 1;  // l etape globale  //12-1-2-3-6-7-15-19-25-32-33
...
private void formCoalitionsProposal(Agent myAgent) throws FIPAException, IOException {
	...
	if(agentsPlansList.size()>=needPlansNbr){
		...
		*/
		...
		discussionList.sort(DESCENDING_COMPARATOR_byEstiCost);
		
		//tempDiscussionList.sort(DESCENDING_COMPARATOR_byIndiCost);
		tempDiscussionList.sort(DESCENDING_COMPARATOR_byEstiCost);
...
private void endingCoalitionFormationProcess(Agent myAgent) throws IOException, SQLException, FIPAException, InterruptedException {
	...
	//  Bloquer les agent apr�s 100 round.
		...
		*/
       // ici faire appel � une version de test
		/*
		...
		*/
		discussionsProbability = Tr.computeDiscussionsProbability(discussionList, sentDiscussionList, actionsFrequencyByAgent, agentsListStr, 
				agentsHaveMyPlan, localPlan, planMgmt,	oldAgentsOrder,  newAgentsOrder, 
				val, newAgentsPropoProfile, oldAgentsPropoProfile, lastSendDiscProfile, globalNegotiationRound, maxRound, agentsPlansList,Ct.referenceCost, Ct.costLimit);
	...
	// Mise � jour de l'ordre des discussions
		...
		if(reorderDiscussionByState2){
			...
			discussionsProbability = Tr.computeDiscussionsProbability(discussionList, sentDiscussionFrequency , actionsFrequencyByAgent, agentsListStr,
					...
					val, newAgentsPropoProfile, oldAgentsPropoProfile, lastSendDiscProfile, globalNegotiationRound, maxRound, agentsPlansList);
			*/
			// ici faire appel a une version de test.
			/*
		discussionsProbability = Tr.computeDiscussionsProbability(discussionList, sentDiscussionList, actionsFrequencyByAgent, agentsListStr, 
				agentsHaveMyPlan, localPlan, planMgmt,	oldAgentsOrder,  newAgentsOrder, 
				val, newAgentsPropoProfile, oldAgentsPropoProfile, lastSendDiscProfile, globalNegotiationRound, maxRound, agentsPlansList);
...
public void saveDiscussionProbabiloities(int round){
	...
	try {
		...
		for(int i=0; i<discussionList.size(); i++){
			...
			fileProbaPrint.println("-----------------------------");
			// to delete -----------------------------
			fileProbaPrint.println("1");
			for(int l=0; l<disc.eCostH.size(); l++){
				fileProbaPrint.print("(eCost,eCostV) :("+disc.eCostH.get(l)+","+disc.eCostV.get(l)+")  ");
			}
			
			fileProbaPrint.println("2");
			for(int l=0; l<disc.wH.size(); l++){
				fileProbaPrint.print("(W,WV) :("+disc.wH.get(l)+","+disc.wV.get(l)+")  ");
			}
			
			fileProbaPrint.println("3");
			for(int l=0; l<disc.rFH.size(); l++){
				fileProbaPrint.print("(rf,rfV) :("+disc.rFH.get(l)+","+disc.rFV.get(l)+")  ");
			}
			
			fileProbaPrint.println("4");
			for(int l=0; l<disc.sFH.size(); l++){
				fileProbaPrint.print("(sf,sfV) :("+disc.sFH.get(l)+","+disc.sFV.get(l)+")  ");
			}
			
			fileProbaPrint.println("5");
			for(int l=0; l<disc.ConvIndH.size(); l++){
				fileProbaPrint.print("(Conv,ConvV) :("+disc.ConvIndH.get(l)+","+disc.ConvIndV.get(l)+")  ");
			}
...
public static final Comparator<Discussion> DESCENDING_COMPARATOR_byEstiCost = new Comparator<Discussion>() {
    // Overriding the compare method to sort the age
	@Override
	public int compare(Discussion o1, Discussion o2) {
		// TODO Auto-generated method stub
		return (int) (o1.discussionEstimatedCost - o2.discussionEstimatedCost);
	}
};
