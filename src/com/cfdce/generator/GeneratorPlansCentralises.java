...
public class GeneratorPlansCentralises {

public static int agentPlanMaxNbr = 100; // le nombre total d'agents 

public static int minStartTasks = 0; // La tache minimale comme dÔøΩbut
public static int maxStartTasks = 4; // la tache maximale comme dÔøΩbut
...
public static int startTasks = 0; // la tache de dÔøΩbut
...
public static int alternativeNbr = 0; // le nombre d'alternatives ÔøΩ mettre dans le plan
...
public static void main(String[] args) throws IOException {
 ...
 //System.out.println("Bonjour, ceci est un gÔøΩnÔøΩrateur de Plans : ");


  // GÔøΩnÔøΩrer un graph  
 ...
 System.out.println("-------------------- Ici");
  ...
  g3.setStrict(false);
  g3.setAutoCreate(true); // crÔøΩation automatique des noeuds.
...
//   T.addAll(n7);
  ...
  if(plan) // si gÔøΩnÔøΩration de plans
  while(planBeginNbr<(agentPlanMaxNbr+1)) {
 ...
 //  System.out.println("The end is: "+(String) T.get(endTasks)+ "       at: "+destinationCounter);
   ...
   //  dijkstra.
    ...
    finalGraph.setAutoCreate( true ); // crÔøΩation automatique des noeuds.
    ...
    if(allPaths.size()>5){   
     ...
     alternativeNbr = ThreadLocalRandom.current().nextInt(4, ((allPaths.size() % 3 )+5));
     //alternativeNbr = 5;
     
     
      System.out.println(" ->Le nombre d'alternatives à mettre dans le plan est :"+alternativeNbr +"         ---> De: "+allPaths.size());
     ...
     while(i1<=alternativeNbr){
      i2++;
      int altNbr =  ThreadLocalRandom.current().nextInt(0, allPaths.size());
     ...
     System.out.println(" Total Edges is ------------------------------------>: "+finalGraph.getEdgeCount());
    ...
    String agColor = getAgentColor("Agent "+planBeginNbr);
      //System.out.println(saveAsMethode(finalGraph, "Plan_"+planNbr, planNbr, "url('file:config/Modele1.css')", agColor));
      saveAsFile(finalGraph, "Plan_"+planBeginNbr, planBeginNbr, "url('file:config/Modele1.css')", agColor);
      
      System.out.println(finalGraph.getId()+" Yes Good, It is a plan -------------------------------------------------------------------------->: "+finalGraph.getEdgeCount());
    ...
    /*
    System.out.println(" Passons ÔøΩ l'autre : ");
  ...
  // GÔøΩnÔøΩration de couts des taches
  //-------------------------------
...
System.out.println(" agentsContainers ");
  ...
  //------------------------------------------------
   ...
   // PrÔøΩparation du plan de l'agent
...
PrintWriter f = null;
try {
 f = new PrintWriter(new FileWriter("AgentsPlans/Plan_Agent_"+planNbr+".txt"));
